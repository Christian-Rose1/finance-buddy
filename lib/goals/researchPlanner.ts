...
export function createResearchPlanner(): ResearchPlanner {
  ...
  if (configured === "ollama") return new FallbackResearchPlanner();
  if (configured === "openrouter") return new OpenRouterResearchPlanner();
  if (configured !== "") {
    throw new Error(`Unsupported research provider: ${configured}`);
  }
